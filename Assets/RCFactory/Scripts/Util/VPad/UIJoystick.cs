...
public class UIJoystick : MonoBehaviour {
	...
	IEnumerator fadeOutJoystick() {
		...
		if (widgetsToFade == null)
			yield break;

		for (int i=0,ii=widgetsToFade.Length; ii>i; ++i)
		{
			Color lastColor = widgetsToFade[i].color;
			Color newColor = lastColor;
			newColor.a = fadeOutAlpha;
			TweenColor.Begin(widgetsToFade[i].gameObject, 0.5f, newColor).method = UITweener.Method.EaseOut;
		}
    }
